...
$enddate=date('Y-m-d', strtotime($_GET['enddate']));
  ...
  $result = mysqli_query($con,"SELECT * FROM impression_daily_report WHERE date BETWEEN '$startdate' AND '$enddate' AND deviceid IN (" . implode(',', $devices) . ") ORDER BY date ASC")or die("Error: ".mysqli_error($con));

 while($row = mysqli_fetch_array($result)) {
 		$deviceid = $row['deviceid'];
 		$impression = $row['impression'];
 		$theresult[$drivermap[$deviceid]]+=$impression;			
 }
 arsort($theresult);
